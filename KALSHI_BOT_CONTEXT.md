...
Update (30-may, attempt #3 — causa identificada): Tercera activación de V2. Reprodujo el desync en bootstrap (T+37s, KXMLB-26-PHI 3c, qty=-11), mismo patrón que attempts 1 y 2. Pero la instrumentación capturó la causa raíz. El logger.error (línea 407) registró: msg_seq=186 state_seq=184 side=yes price_cents=3 delta_size=-13 bucket_qty_pre_delta=2. El book estaba en seq 184 y llegó un delta de seq 186 — falta el 185. V2 aplicó un delta adelantado sobre estado desincronizado: 2 + (-13) = -11 → OrderbookDesyncError. NO es feed corruption (el label en el mensaje de error es del código, no diagnóstico); es bug interno de V2 en el manejo de gap/recovery durante bootstrap. Hipótesis C del tercer discovery, confirmada con evidencia dura. A (feed) y B (snapshot parcial) quedan descartadas. Recovery no convergió (books_initialized: 0 a T+6min). Rollback <5min, V1 sano. Evidencia: data/v2_attempt3_20260530_174849.log (390 líneas). Estado del frente V2: de "causa NO resuelta" a "causa identificada, fix pendiente". Próximo paso: cuarto discovery → diseño de fix para gap-handling en bootstrap (problema concreto, ya no fantasma).
