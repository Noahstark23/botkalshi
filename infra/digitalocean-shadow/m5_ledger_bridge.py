...
def _read_row(source: Path, experiment_id: str, fill_id: int) -> dict[str, Any]:
    ...
    with closing(sqlite3.connect(uri, uri=True, isolation_level=None, timeout=3)) as con:
        ...
        try:
            kinds = con.execute(
                "SELECT type FROM sqlite_master WHERE name=?", ("mm_shadow_fills",)
            ).fetchall()
            if kinds != [("table",)]:
...
def _result() -> dict[str, Any]:
    return {
        "schema": "botkalshi-m5-ledger-bridge-v1",
        "status": "BLOCKED",
        "reason_codes": [],
        "review": None,
        "reservation": None,
    ...
    *,
    ...
    try:
        booked = simulation_bank.reserve_with_evidence_outcome(
            ...
            amount_usd=durable["max_loss_usd"],
            evidence=evidence,
        )
