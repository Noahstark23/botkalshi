...
def _h2h_outcome_names(odds_event: OddsEvent) -> list[str]:
    """Delegado al núcleo puro (`consensus.h2h_outcome_names`, extraído 2026-09-22)."""
    return h2h_outcome_names(odds_event)
...
def _consensus_fair_probs(
    ...
    *,
    ...
    """
    Probabilidad JUSTA por outcome canónico (mediana de casas, sin vig). La matemática
    vive en `consensus.consensus_fair_probs` (núcleo puro, compartido con research);
    acá quedan el logging y `provenance_out`, idénticos a antes de la extracción.
    """
    fair, stats = consensus_fair_probs(
        odds_event, min_books=min_books, max_book_age_min=max_book_age_min, now=now
    )
    if stats["reason"] == "no_h2h_reference":
        return {}
    if stats["skipped_books"] or stats["stale_books"] or stats["unknown_age_books"]:
        logger.info(
            f"motor2.consensus.books_descartadas event={getattr(odds_event, 'id', '?')} "
            f"set_distinto={stats['skipped_books']} stale={stats['stale_books']} "
            f"sin_timestamp={stats['unknown_age_books']}"
        )
    if stats["reason"] == "too_few_books":
        logger.info(
            f"motor2.consensus.pocas_casas event={getattr(odds_event, 'id', '?')} "
            f"n_books={stats['n_books']} < min={min_books} → sin fair (consenso degenerado)"
        )
    if not fair:
        return {}
    if provenance_out is not None:
        provenance_out.update(
            bookmaker_keys=stats["bookmaker_keys"],
            oldest_book_update=stats["oldest_book_update"],
            newest_book_update=stats["newest_book_update"],
        )
    logger.info(
        f"motor2.consensus event={getattr(odds_event, 'id', '?')} n_books={stats['n_books']} "
        f"outcomes={len(fair)} fair={ {n: round(p, 4) for n, p in fair.items()} }"
    )
    return fair
...
def _select_candidate(
    ...
    """Delegado al núcleo puro (`consensus.select_candidate`, extraído 2026-09-22)."""
    return select_candidate(event_key, candidates, now)
