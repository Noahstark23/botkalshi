...
class RestKalshiQuoteSource:
    ...
    async def fetch(self) -> list[KalshiEventQuotes]:
        ...
        if not universe:
            logger.info("motor2.kalshi universo vacío (discovery aún sin partidos en MOTOR2_SERIES)")
            return []
        ...
        logger.info(f"motor2.kalshi {len(out)}/{len(universe)} eventos con quotes usables (2-way/1X2)")
        return out
