...
def _insert_trade(
    *, strategy: str, status: str, side: str = "yes", ticker: str = "KXT",
    price: int = 40, count: int = 10, pnl: int | None = None,
    settled: bool = False, arb_id: str | None = None,
) -> None:
    with models.get_session() as s:
        s.add(models.Trade(
            client_order_id=f"{arb_id}-{side}" if arb_id else str(uuid.uuid4()),
            ticker=ticker, side=side, action="buy", count=count, price_cents=price,
            strategy=strategy, status=status, pnl_cents=pnl,
...
def test_complete_hedged_arb_is_discounted_to_zero():
    ...
    arb = str(uuid.uuid4())
    _insert_trade(strategy="motor_rest_arb", status="filled", side="yes",
                  ticker="KXA", price=40, count=5, arb_id=arb)
    _insert_trade(strategy="motor_rest_arb", status="filled", side="no",
                  ticker="KXA", price=45, count=5, arb_id=arb)
...
def test_lone_filled_leg_counts_fully():
    ...
    _insert_trade(strategy="motor_rest_arb", status="filled", side="yes",
                  ticker="KXB", price=40, count=5, arb_id=str(uuid.uuid4()))
...
def test_partial_hedge_discounts_only_paired_contracts():
    ...
    arb = str(uuid.uuid4())
    _insert_trade(strategy="motor_rest_arb", status="filled", side="yes",
                  ticker="KXC", price=40, count=10, arb_id=arb)   # $4.00
    _insert_trade(strategy="motor_rest_arb", status="filled", side="no",
                  ticker="KXC", price=45, count=5, arb_id=arb)    # $2.25
...
def test_pending_legs_are_never_discounted():
    ...
    arb = str(uuid.uuid4())
    _insert_trade(strategy="motor_rest_arb", status="pending", side="yes",
                  ticker="KXD", price=40, count=5, arb_id=arb)
    _insert_trade(strategy="motor_rest_arb", status="pending", side="no",
                  ticker="KXD", price=45, count=5, arb_id=arb)
...
def test_cross_ticker_legs_not_treated_as_hedge():
    ...
    arb = str(uuid.uuid4())
    _insert_trade(strategy="motor_rest_arb", status="filled", side="yes",
                  ticker="KXE", price=40, count=5, arb_id=arb)
    _insert_trade(strategy="motor_rest_arb", status="filled", side="no",
                  ticker="KXF", price=45, count=5, arb_id=arb)
    assert _manager()._get_current_exposure_usd() == pytest.approx(4.25)
...
async def test_concurrent_checks_are_serialized_across_instances():
    ...
    with patch.object(mgr_a, "_check_timeframe_stop_losses", new=make_slow_stop_losses()), \
